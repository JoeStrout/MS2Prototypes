...
unsigned long UnicodeCharToUpper(unsigned long lower) {
    if (lower > 0xFFFF) return lower; // Only handle BMP (Basic Multilingual Plane)
    return findLowerInUpperTable((unsigned short)lower);
}

unsigned long UnicodeCharToLower(unsigned long upper) {
    if (upper > 0xFFFF) return upper; // Only handle BMP (Basic Multilingual Plane)  
    return findUpperInLowerTable((unsigned short)upper);
}
...
// UTF8ToUpper
//
//	Converts a UTF8 String into uppercase.  Note that the result is a freshly
//  allocated (with malloc) buffer, which the caller must adopt or free().
//
...
// Returns: true if any characters were changed, false if output is identical to input.
bool UTF8ToUpper( unsigned char *utf8String, unsigned long byteCount,
				  unsigned char **outBuf, unsigned long *outByteCount ) {
	if (!outBuf && !outByteCount) return !UTF8IsCaseless(utf8String, byteCount);
	...
	while (utf8String != endOfBuffer) {
		...
		unsigned long oldChar = UTF8Decode(utf8String);
		unsigned long newChar = UnicodeCharToUpper(oldChar);
		anyChanged |= (newChar != oldChar);
		
		//	Sanity Check: make sure we're not writing out beyond the end of our allocated output buffer
		if (curOutPos > endOfOutputBuffer) break;
		
		//	Encode the new letter, and advance out return string's buffer
		curOutPos += UTF8Encode(newChar, curOutPos);
				  ...
				  unsigned char **outBuf, unsigned long *outByteCount )
{
	...
	while (utf8String != endOfBuffer) {
		...
		unsigned long oldChar = UTF8Decode(utf8String);
		unsigned long newChar = UnicodeCharToLower(oldChar);
		anyChanged |= (newChar != oldChar);
				  ...
				  unsigned char **outBuf, unsigned long *outByteCount )
{
	...
	while (utf8String != endOfBuffer) {
		...
		unsigned long oldChar = UTF8Decode(utf8String);
		unsigned long newChar = atStartOfWord ? UnicodeCharToUpper(oldChar) :
								UnicodeCharToLower(oldChar);
		anyChanged |= (newChar != oldChar);
		...
		atStartOfWord = UnicodeCharIsWhitespace(newChar);
...
bool UTF8IsCaseless(unsigned char *utf8String, unsigned long byteCount)
{
	...
	while (ptr < pend) {
		...
		if (uchar > 0xFFFF) continue;
		if (findLowerInUpperTable(uchar) != uchar || findUpperInLowerTable(uchar) != uchar) {
			return false;
		}
	}
	return true;
					   ...
					   unsigned char *rightBuf, unsigned long rightByteCount)
{
	...
	while (lptr < lend && rptr < rend) {
		...
		long comparison = UnicodeCharCompare(lchar, rchar);
