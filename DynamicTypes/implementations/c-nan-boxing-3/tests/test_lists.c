...
void test_list_add_get() {
    ...
    GC_LOCALS(list, item0, item1, item2, invalid, negative);
    list = make_list(10);
    ...
    // Test out of bounds
    invalid = list_get(list, 10);
    assert(is_nil(invalid));
    
    negative = list_get(list, -1);
...
void test_list_indexOf() {
    ...
    GC_LOCALS(list, str1, str2, str3, search1, search2, int_search, int_missing);
    list = make_list(10);
    
    str1 = make_string("apple");
    str2 = make_string("banana");
    str3 = make_string("cherry");
    ...
    search1 = make_string("banana");
    search2 = make_string("grape"); // Not in list
    
    assert(list_indexOf(list, search1) == 2);
    assert(list_indexOf(list, search2) == -1);
    
    // Test integer search
    int_search = make_int(100);
    assert(list_indexOf(list, int_search) == 1);
    
    int_missing = make_int(999);
...
void test_list_capacity_bounds() {
    ...
    GC_LOCALS(small_list);
    small_list = make_list(2);
...
void test_benchmark_operations() {
    ...
    GC_LOCALS(singles_str, space, singles_list, search, found_item, word, char_delim, char_list, first_char, last_char);
    ...
    found_item = list_get(singles_list, idx);
    assert(is_string(found_item));
    assert(strcmp(as_cstring(found_item), "five") == 0);
    
    // Test levenshtein-style character split
    word = make_string("kitten");
    char_delim = make_string("");
    char_list = string_split(word, char_delim);
    
    assert(is_list(char_list));
    assert(list_count(char_list) == 6); // k-i-t-t-e-n
    
    first_char = list_get(char_list, 0);
    last_char = list_get(char_list, 5);
