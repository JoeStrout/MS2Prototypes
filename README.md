...
- MS2Proto3: our newest VM prototype, focused on simultaneous development of C# and C++ code via transpiler.ms.
- MS2Proto4: a small Pratt parser prototype; also our first to embrace smart pointers (introduced in C++11, and improved in C++14) for memory management on the C++ side.
- MS2Proto5: same as MS2Proto4, but we've removed the old MemPool stuff completely, and simplified the parser so that instead of performing arbitrary operations, it always generates an AST.
